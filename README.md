...
Composant retenu : `icon-tile` (primitives, dépend de `expo-linear-gradient`, compatible Expo Go). La plupart des autres dépôts de [rit3zh](https://github.com/rit3zh) (hors `reacticx`) sont des modules natifs **iOS uniquement**, écrits en Swift : à éviter pour une cible Android.

**Écarté : tout composant qui dépend de `react-native-reanimated`** (`otp-input` inclus), à cause d'une incompatibilité constatée en marge du ticket #26 : Reanimated v4 (et son paquet `react-native-worklets`) ne fonctionne pas sous Jest avec `expo-router/testing-library`. Le chargement des routes de ce dernier (son « ponyfill » de `require.context`, pour simuler le routage par fichiers de Metro) contourne toutes les doublures de test possibles (`moduleNameMapper`, `jest.mock()`, `__mocks__/`) : le vrai module natif se charge quand même et plante (aucun native binaire compilé sous Jest). Le champ de code à 6 chiffres (`src/screens/profil/champ-code.tsx`) et les notifications éphémères (`src/lib/toasts.ts`, `src/components/toast-host.tsx`) sont donc faits main, avec l'API `Animated` native de React Native (pas de module natif, aucun souci sous Jest).
